    ...
    const bcmolt_group_descr *key)
{
    ...
    if (flags != DONT_SERIALIZE)
    {
        ...
        {
            err = bcmolt_type_get_serialized_length(group->type, flags, MSG_FIELD_PTR(msg, group->data_offset), &len);
            if (err != BCM_ERR_OK)
            {
                return err;
            }
...
{
    bcmolt_serializer_flag flags;
    bcmos_errno err = bcmolt_type_serialize(
        key->type,
        BCMOLT_SERIALIZER_FLAG_NONE,
        MSG_FIELD_PTR(msg, group->key_offset),
        buf);
    ...
    if (flags != DONT_SERIALIZE)
    {
        ...
        {
            err = bcmolt_type_serialize(group->type, flags, MSG_FIELD_PTR(msg, group->data_offset), buf);
            if (err != BCM_ERR_OK)
            {
                return err;
            }
    ...
    uint32_t *size)
{
    ...
    err = bcmolt_type_extra_mem_scan(key->type, BCMOLT_SERIALIZER_FLAG_NONE, buf, size);
    if (err != BCM_ERR_OK)
    {
        return err;
    }
    ...
    {
        err = bcmolt_type_extra_mem_scan(group->type, flags, buf, size);
        if (err != BCM_ERR_OK)
        {
            return err;
        }
    ...
    const bcmolt_group_descr *key)
{
    ...
    **unpacked = *hdr;
    ...
    err = bcmolt_type_deserialize(key->type, BCMOLT_SERIALIZER_FLAG_NONE, key_ptr, buf, list_mem_buf_ptr);
    if (err != BCM_ERR_OK)
    {
        if (did_malloc)
        {
    ...
    if (flags != DONT_SERIALIZE && group->type != NULL)
    {
        ...
        err = bcmolt_type_deserialize(group->type, flags, data_ptr, buf, list_mem_buf_ptr);
        if (err != BCM_ERR_OK)
        {
            if (did_malloc)
            {
    ...
    const bcmolt_group_descr *key)
{
    ...
    err = bcmolt_type_deserialize(key->type, BCMOLT_SERIALIZER_FLAG_NONE, key_buf, buf, NULL);
    if (err != BCM_ERR_OK)
    {
        return err;
    }
    ...
    if (multi->hdr.dir == BCMOLT_MSG_DIR_REQUEST)
    {
        ...
        if ((multi->filter_flags & BCMOLT_FILTER_FLAGS_DISABLE_FILTER) == 0)
        {
            ...
            if (cfg_group != NULL)
            {
                ...
                err = bcmolt_type_deserialize(cfg_group->type, BCMOLT_SERIALIZER_FLAG_NONE, filter, buf, NULL);
                if (err != BCM_ERR_OK)
                {
                    goto done;
                }
...
bcmos_errno bcmolt_msg_unpack(bcmolt_buf *buf, bcmolt_msg **unpacked)
{
    ...
    return (hdr.type & BCMOLT_MSG_TYPE_MULTI) == 0
